...
public class ReportDataService {
    ...
    public WaterBalanceReportDTO getWaterBalanceData(Integer farmId, Date startDate, Date endDate,
        ...
        List<Irrigation> irrigations = irrigationRepository.findBySectorInAndStartDatetimeBetween(targetSectors,
                startDate, endDate);
        List<Precipitation> precipitations = precipitationRepository.findByFarm_IdAndPrecipitationDateBetween(farmId,
                startDate, endDate);
        ...
        for (Sector sector : targetSectors) {
            ...
            summary.setTotalIrrigationWater(dailyDataList.stream()
                    .map(WaterBalanceReportDTO.DailyData::getIrrigationWater).reduce(BigDecimal.ZERO, BigDecimal::add));
            summary.setTotalEffectiveRain(dailyDataList.stream().map(WaterBalanceReportDTO.DailyData::getEffectiveRain)
                    .reduce(BigDecimal.ZERO, BigDecimal::add));
            summary.setTotalIrrigationHours(
                    irrigations.stream().filter(i -> i.getSector().getId().equals(sector.getId()))
                            .map(Irrigation::getIrrigationHours).reduce(BigDecimal.ZERO, BigDecimal::add));
        ...
        farmTotals.setTotalIrrigationWater(sectorDataList.stream().map(s -> s.getSummary().getTotalIrrigationWater())
                .reduce(BigDecimal.ZERO, BigDecimal::add));
        farmTotals.setTotalEffectiveRain(dailyRainMap.values().stream().reduce(BigDecimal.ZERO, BigDecimal::add));
        farmTotals.setTotalIrrigationHours(sectorDataList.stream().map(s -> s.getSummary().getTotalIrrigationHours())
                .reduce(BigDecimal.ZERO, BigDecimal::add));
    ...
    public OperationsLogReportDTO getOperationsLogData(Integer farmId, Date startDate, Date endDate,
        ...
        irrigationRepository.findBySector_Farm_IdAndStartDatetimeBetween(farmId, startDate, endDate).forEach(i -> {
            ...
            op.setType("RIEGO");
            op.setDescription(
                    String.format("Riego: %.2f m³ en %.2f horas.", i.getWaterAmount(), i.getIrrigationHours()));
        ...
        maintenanceRepository.findAll().stream()
                .filter(m -> m.getIrrigationEquipment().getFarm().getId().equals(farmId)
                        && !m.getDate().before(startDate) && !m.getDate().after(endDate))
                .forEach(m -> {
                    OperationsLogReportDTO.Operation op = new OperationsLogReportDTO.Operation();
        ...
        fertilizationRepository.findAll().stream()
                .filter(f -> f.getSector().getFarm().getId().equals(farmId) && !f.getDate().before(startDate)
                        && !f.getDate().after(endDate))
                .forEach(f -> {
                    OperationsLogReportDTO.Operation op = new OperationsLogReportDTO.Operation();
                    op.setDatetime(sdf.format(f.getDate()));
                    op.setType("FERTILIZACION");
                    op.setDescription(
                            String.format("%s: %.2f %s", f.getFertilizerType(), f.getQuantity(), f.getQuantityUnit()));
        ...
        dateRange.setEnd(sdf.format(endDate));
        report.setDateRange(dateRange);
        report.setOperations(
                stream.sorted(Comparator.comparing(OperationsLogReportDTO.Operation::getDatetime).reversed())
    ...
    public PeriodSummaryReportDTO getPeriodSummaryData(Integer farmId, Date startDate, Date endDate) {
        ...
        List<Irrigation> irrigations = irrigationRepository.findBySector_Farm_IdAndStartDatetimeBetween(farmId,
                startDate, endDate);
        waterSummary.setTotalIrrigationWaterM3(
                irrigations.stream().map(Irrigation::getWaterAmount).reduce(BigDecimal.ZERO, BigDecimal::add));
        waterSummary.setTotalIrrigationHours(
                irrigations.stream().map(Irrigation::getIrrigationHours).reduce(BigDecimal.ZERO, BigDecimal::add));

        List<Precipitation> precipitations = precipitationRepository.findByFarm_IdAndPrecipitationDateBetween(farmId,
                startDate, endDate);
        waterSummary.setTotalPrecipitationMM(
                precipitations.stream().map(Precipitation::getMmRain).reduce(BigDecimal.ZERO, BigDecimal::add));
        waterSummary.setTotalEffectivePrecipitationMM(precipitations.stream().map(Precipitation::getMmEffectiveRain)
                .reduce(BigDecimal.ZERO, BigDecimal::add));
        ...
        opsSummary.setTasksCompleted(taskRepository.countBySector_Farm_IdAndCreatedAtBetweenAndStatus(farmId, startDate,
                endDate, TaskStatus.COMPLETADA));
        opsSummary.setMaintenanceRecords(
                maintenanceRepository.countByIrrigationEquipment_Farm_IdAndDateBetween(farmId, startDate, endDate));
        opsSummary.setFertilizationRecords(
                fertilizationRepository.countBySector_Farm_IdAndDateBetween(farmId, startDate, endDate));
