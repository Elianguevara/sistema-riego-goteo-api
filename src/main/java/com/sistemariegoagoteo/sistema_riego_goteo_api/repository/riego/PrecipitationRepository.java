...
public interface PrecipitationRepository extends JpaRepository<Precipitation, Integer> {
        ...
        List<Precipitation> findByPrecipitationDate(LocalDate date);

        List<Precipitation> findByFarmAndPrecipitationDateBetween(Farm farm, LocalDate startDate, LocalDate endDate);

        List<Precipitation> findByFarmOrderByPrecipitationDateDesc(Farm farm);

        Optional<Precipitation> findByFarmAndPrecipitationDate(Farm farm, LocalDate date);

        @Query("SELECT COALESCE(SUM(p.mmRain), 0) FROM Precipitation p WHERE p.farm = :farm AND p.precipitationDate = :date")
        BigDecimal getDailyPrecipitation(@Param("farm") Farm farm, @Param("date") LocalDate date);
        ...
        BigDecimal getMonthlyPrecipitation(@Param("farm") Farm farm, @Param("year") int year,
        ...
        BigDecimal getAnnualPrecipitation(@Param("farm") Farm farm, @Param("startDate") LocalDate startDate,
         ...
         * fechas.
         */
        List<Precipitation> findByFarm_IdAndPrecipitationDateBetween(Integer farmId, LocalDate startDate,
                        LocalDate endDate);
        ...
        List<com.sistemariegoagoteo.sistema_riego_goteo_api.dto.report.projection.DailyRainProjection> findDailyRainByFarm(
                        @Param("farmId") Integer farmId, @Param("startDate") LocalDate startDate,
                        @Param("endDate") LocalDate endDate);

        @Query("SELECT p.precipitationDate, SUM(p.mmEffectiveRain) FROM Precipitation p " +
                        "WHERE p.farm.id = :farmId AND p.precipitationDate BETWEEN :startDate AND :endDate " +
                        "GROUP BY p.precipitationDate")
        List<Object[]> findDailyEffectiveRainByFarm(@Param("farmId") Integer farmId,
